    ...
    Instructions.Nullary i ->
	  ...
	  case value of
        ...
        Instructions.Write  -> do
          ...
          put $ machine { rpc = rpc + 1, rtp = rtp - 1 }
          run
        -- Added in order to print strings out
		-- converts the memory from an array to a list
		-- then I am able to sort the last N instructions
		-- using drop and take, then since the instructions
		-- are in reverse order, I use a reverse and then 
		-- use the tell monad to print the instruction list.
        Instructions.WriteStr  -> do
          let size = smem ! (rtp - 1)
          let list = elems smem
          let theRtp = take ((fromIntegral rtp) - 1) list
          let theStackNeeded = drop ( (fromIntegral rtp) - (fromIntegral size) - 1) theRtp
          let resultList =  reverse ( map (chr . fromIntegral) theStackNeeded )
          tell $ [resultList]
          put $ machine {rpc = rpc +1 , rtp = rtp - 1 - (fromIntegral size)}
          run
